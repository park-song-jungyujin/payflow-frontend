// 최소 i18n. 라이브러리 없이 정적 딕셔너리 + 쿠키만으로 구현한다(해커톤
// 범위 — next-intl 같은 라우팅 기반 i18n은 지금 필요한 것보다 크다).
// 기본값은 en이다(영어권 사용자가 1순위, 오른쪽 위 버튼으로 ko 전환).
// 서버 컴포넌트는 lib/locale.ts의 getLocale()(next/headers 씀)로 쿠키를 읽고,
// 클라이언트 컴포넌트는 그 값을 prop으로 내려받는다 — next/headers는 클라
// 컴포넌트에서 import 자체가 안 되므로 이 파일은 next/headers에 의존하지 않는다.

export const LOCALES = ["en", "ko"] as const;
export type Locale = (typeof LOCALES)[number];
export const DEFAULT_LOCALE: Locale = "en";
export const LOCALE_COOKIE = "locale";

export const dict = {
  en: {
    subtitle: "Settlement Admin Dashboard",
    runListTitle: "Settlement Runs",
    noRuns: "No settlement runs yet.",
    colNo: "No.",
    colStatus: "Status",
    colAmount: "Amount",
    colRequester: "Requester",
    detailLink: "View",
    newRunTitle: "New Settlement Run",
    periodStart: "Start date",
    periodEnd: "End date",
    categoriesLegend: "Account categories (all if none selected)",
    creating: "Creating...",
    createRun: "Create Run",
    createFailed: (status: number) => `Failed to create run (${status})`,
    createNetworkError: "Network error while creating the run.",
    backToList: "← Back to list",
    statusLabel: "Status",
    totalLabel: "Total",
    draftHint: "(Provisional total — recalculated with the exchange rate at approval time.)",
    claimsTitle: "Claim Details",
    noClaims: "No claims linked to this run.",
    itemIncludeLabel: (name: string) => `Include ${name} in settlement amount`,
    itemToggleFailed: (status: number) => `Failed to update item (${status})`,
    itemToggleNetworkError: "Network error while updating the item.",
    colMerchant: "Merchant",
    colTxDate: "Date",
    colCategory: "Category",
    anomalyTitle: "Anomaly Review (Executor Agent)",
    analysisPending: "Analysis pending",
    analysisFailed: "Could not start analysis — review manually",
    noAnomalies: "No anomalies found",
    retryAnalysisButton: "Retry",
    retryingAnalysis: "Retrying...",
    retryAnalysisFailed: (status: number) => `Retry failed (${status})`,
    retryAnalysisNetworkError: "Network error while retrying the analysis.",
    exportLink: "Download XLSX for accountant",
    approveTitle: "Approval",
    approveButton: "Approve & Send Payout",
    approving: "Processing...",
    approveFailed: (status: number) => `Approval failed (${status})`,
    approveNetworkError: "Network error during approval.",
    noRecipientsReason: "No linked claims — cannot approve.",
    analysisInProgressReason: "Waiting for anomaly analysis to finish before approval.",
    excludeClaimButton: "Exclude",
    excluding: "Excluding...",
    excludeClaimConfirm: "Exclude this claim from the settlement run? It will return to unsettled claims.",
    excludeClaimFailed: (status: number) => `Failed to exclude claim (${status})`,
    showAll: (n: number) => `Show all (${n})`,
    unsettledClaimsTitle: "Unsettled Claims",
    noUnsettledClaims: "No unsettled claims — all caught up.",
    slackConnectTitle: "Slack Workspace",
    slackConnectButton: "Add to Slack",
    loggedInAs: (org: string, email: string) => `${org} · ${email}`,
    logout: "Log out",
    loginTitle: "Sign in to Payflow",
    loginSubtitle: "Sign in with your Google account to manage expense settlements and automated payouts.",
    orgNameLabel: "Organization Name",
    orgNameBadge: "First-time sign up",
    orgNamePlaceholder: "e.g. Acme Corp or Team Alpha",
    orgNameHint: "Only needed on first sign-up. If you already belong to an organization, this will be linked automatically.",
    continueWithGoogle: "Continue with Google",
    missingClientId: "Google Client ID (NEXT_PUBLIC_GOOGLE_CLIENT_ID) is not configured.",
    loginErrorTitle: "Sign-in Failed",
    securityNotice: "Enterprise Financial Safety & IAM Isolation",
    privacyPolicy: "Privacy Policy",
    support: "Support",
  },
  ko: {
    subtitle: "정산 관리자 대시보드",
    runListTitle: "정산 실행 목록",
    noRuns: "정산 실행이 없습니다.",
    colNo: "번호",
    colStatus: "상태",
    colAmount: "총액",
    colRequester: "요청자",
    detailLink: "상세 보기",
    newRunTitle: "새 정산 실행",
    periodStart: "시작일",
    periodEnd: "종료일",
    categoriesLegend: "계정과목 (선택 안 하면 전체)",
    creating: "생성 중...",
    createRun: "정산 실행 생성",
    createFailed: (status: number) => `실행 생성 실패 (${status})`,
    createNetworkError: "실행 생성 중 네트워크 오류가 발생했습니다.",
    backToList: "← 목록으로",
    statusLabel: "상태",
    totalLabel: "총액",
    draftHint: "(승인 전 잠정치 — 승인 시점 환율로 다시 계산됩니다)",
    claimsTitle: "정산 명세",
    noClaims: "연결된 청구 항목이 없습니다.",
    itemIncludeLabel: (name: string) => `${name}을(를) 정산 금액에 포함`,
    itemToggleFailed: (status: number) => `물품 상태 변경 실패 (${status})`,
    itemToggleNetworkError: "물품 상태 변경 중 네트워크 오류가 발생했습니다.",
    colMerchant: "가맹점",
    colTxDate: "거래일",
    colCategory: "계정과목",
    anomalyTitle: "이상 징후 (집행자 에이전트)",
    analysisPending: "분석 대기 중",
    analysisFailed: "분석을 시작하지 못했습니다 — 직접 확인해주세요",
    noAnomalies: "이상 없음",
    retryAnalysisButton: "재시도",
    retryingAnalysis: "재시도 중...",
    retryAnalysisFailed: (status: number) => `재시도 실패 (${status})`,
    retryAnalysisNetworkError: "재시도 중 네트워크 오류가 발생했습니다.",
    exportLink: "세무사 전달용 XLSX 다운로드",
    approveTitle: "승인",
    approveButton: "최종 승인 및 송금 실행",
    approving: "처리 중...",
    approveFailed: (status: number) => `승인 실패 (${status})`,
    approveNetworkError: "승인 처리 중 네트워크 오류가 발생했습니다.",
    noRecipientsReason: "연결된 청구 항목이 없어 승인할 수 없습니다.",
    analysisInProgressReason: "이상 징후 분석이 끝날 때까지 승인할 수 없습니다.",
    excludeClaimButton: "제외",
    excluding: "제외 중...",
    excludeClaimConfirm: "이 청구를 정산 실행에서 제외할까요? 미정산 청구 목록으로 돌아갑니다.",
    excludeClaimFailed: (status: number) => `청구 제외 실패 (${status})`,
    showAll: (n: number) => `모두 보기 (${n})`,
    unsettledClaimsTitle: "미정산 항목",
    noUnsettledClaims: "미정산 항목이 없습니다.",
    slackConnectTitle: "Slack 워크스페이스",
    slackConnectButton: "Slack에 앱 추가",
    loggedInAs: (org: string, email: string) => `${org} · ${email}`,
    logout: "로그아웃",
    loginTitle: "Payflow 로그인",
    loginSubtitle: "Google 계정으로 로그인하여 영수증 정산 및 자동 송금을 관리하세요.",
    orgNameLabel: "기관명",
    orgNameBadge: "최초 가입 시",
    orgNamePlaceholder: "예: (주)페이플로우 또는 프로젝트팀",
    orgNameHint: "처음 로그인할 때 새 기관이 생성됩니다. 이미 등록된 계정은 자동으로 소속 기관에 연결됩니다.",
    continueWithGoogle: "Google 계정으로 계속하기",
    missingClientId: "Google 클라이언트 ID(NEXT_PUBLIC_GOOGLE_CLIENT_ID)가 설정되지 않았습니다.",
    loginErrorTitle: "로그인 실패",
    securityNotice: "엔터프라이즈급 금융 안전 및 IAM 격리 보호",
    privacyPolicy: "개인정보처리방침",
    support: "고객지원",
  },
} as const satisfies Record<
  Locale,
  Record<
    string,
    | string
    | ((n: number) => string)
    | ((org: string, email: string) => string)
    | ((name: string) => string)
  >
>;

export function t(locale: Locale) {
  return dict[locale];
}
